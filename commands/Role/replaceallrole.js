...
async function action(client, guild, search, replace, author) {
    ...
    await asyncForEach([...guild.roles.cache.values()],async role => {
        if (role.name.replace(search, replace).length < 100)
            await role.setName(role.name.replace(search, replace), `Configuration demandé par ${author.tag}`)
                .then(() => changed++)
                .catch(e => {
                    client.log("error", e)
                    unchanged++
...
module.exports.help = {
    ...
    categorie: 'roles',
    description: 'modifie une valeur par une autre valeur dans tous les rôles',
    usage: '<valeur> [autre_valeur]',
    options: [
        {
